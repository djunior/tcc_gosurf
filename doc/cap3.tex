...
\paragraph{}No projeto de graduação as imagens foram adquiridas de duas formas: através de câmeras de \textit{smartphones} comuns e através da câmera montada no \textit{hardware} apresentado anteriormente. Embora para este projeto a câmera de \textit{smartphones} atenda o requisito, o aparato instrumental é fundamental para o projeto \textit{GoSurf} completo porque agrega sensores extras além da câmera. Outra vantagem do aparato sensorial é a capacidade de realizar o pré-processamento localmente, o que reduz o volume de dados que devem ser enviado ao servidor. Por exemplo, um vídeo de 125MB é reduzido para uma imagem de 657KB.

\paragraph{}A central de processamento utilizada no projeto GoSurf e no projeto de graduação é um Raspberry Pi, um pequeno computador implementado em uma única placa. O Raspberry Pi é um computador completo, sendo possível instalar diversos sistemas operacionais diferentes. Neste projeto seguiu-se a recomendação oficial e foi utilizado o sistema operacional Raspbian - um sistema operacional Linux baseado em Debian. Por ser baseado em Linux, pode-se utilizar a biblioteca de processamento gráfico OpenCV para agilizar a implementação do projeto de graduação.

\paragraph{}Um dos motivos para a escolha do Raspberry Pi foi a facilidade de integrar com uma câmera. Existe um módulo de câmera oficial para o Raspberry Pi \cite{RaspberryCamera}, que foi a câmera escolhida para este projeto. Utilizando este módulo e o sistema operacional Raspbian, a integração com o projeto é trivial, basta conectar a câmera à central de processamento. Esta câmera é capaz de capturar vídeos com resolução 1080x720 a 30 \textit{frames} por segundo, a mesma qualidade de vídeo utilizada em capturas de vídeo com \textit{smartphones}. O módulo de câmera possui ponto focal fixo na faixa de 1 metro até o infinito, o que não é um problema para o algoritmo pois os objetos de estudo, as ondas do mar, estão localizados nesta faixa. O campo de visão vertical desta câmera é de 41.41 graus (mais ou menos 0.11 graus). Este dado será importante durante a conversão da altura das ondas estimada em \textit{pixels} para metros.
...
\paragraph{}A equalização de histograma tem como função normalizar o efeito da iluminação em uma imagem. As figuras \ref{FigHistogram}a, \ref{FigHistogram}b, \ref{FigHistogram}c e \ref{FigHistogram}d ilustram o efeito da equalização de histograma sobre uma imagem de alta luminosidade, enquanto as figuras \ref{FigHistogram}e, \ref{FigHistogram}f, \ref{FigHistogram}g e \ref{FigHistogram}h ilustram o efeito da equalização de histograma em um dia de baixa luminosidade. Nota-se que o histograma do dia com baixa luminosidade apresenta uma grande concentração de \textit{pixels} nos níveis de maior intensidade, enquanto o histograma do dia com alta luminosidade está melhor distribuído por todo o espectro de níveis de cinza. As figuras \ref{FigHistogram}c e \ref{FigHistogram}g exibem o efeito de normalização da luminosidade no vídeo. Pode-se notar que o efeito da equalização foi mais significativo na figura \ref{FigHistogram}g.

\begin{figure}
  \centering
  \subfloat[\small{Imagem de uma praia em dia ensolarado (alta luminosidade).}]{\includegraphics[width=.45\textwidth,height=.1\textheight]{sunny_grey_image.jpg}}
  \qquad
  \subfloat[\small{Histograma da imagem original.}]{\includegraphics[width=.3\textwidth,height=.1\textheight]{sunny_histogram.jpg}}
  \qquad
  \subfloat[\small{Resultado da equalização da imagem original.}]{\includegraphics[width=.45\textwidth,height=.1\textheight]{sunny_equalized_image.jpg}}
  \qquad
  \subfloat[\small{Histograma da imagem equalizada.}]{\includegraphics[width=.3\textwidth,height=.1\textheight]{sunny_histogram_eq.jpg}}
  \qquad
  \subfloat[\small{Imagem de uma praia em dia nublado (baixa luminosidade).}]{\includegraphics[width=.45\textwidth,height=.1\textheight]{cloud_grey_image.jpg}}
  \qquad
  \subfloat[\small{Histograma da imagem original.}]{\includegraphics[width=.3\textwidth,height=.1\textheight]{cloud_histogram.jpg}}
  \qquad
  \subfloat[\small{Resultado da equalização da imagem original.}]{\includegraphics[width=.45\textwidth,height=.1\textheight]{cloud_equalized_image.jpg}}
  \qquad
  \subfloat[\small{Histograma da imagem equalizada.}]{\includegraphics[width=.3\textwidth,height=.1\textheight]{cloud_histogram_eq.jpg}}
  \caption[\small{Comparação entre o histograma original e equalizado de um dia ensolarado.}]{\small{Comparação entre o histograma original e equalizado de um dia ensolarado.}}
  \label{FigHistogram}
\end{figure}

\paragraph{}A iluminação de um vídeo capturado em ambiente externo pode variar segundo alguns fatores:

\begin{enumerate}
  \item O horário do dia.
  \item A condição climática do dia.
  \item Nuvens se deslocando sob o Sol ou objetos externos produzindo sombra sobre a câmera.
\end{enumerate}

\noindent{}O efeitos do horário e condição climática do dia em geral permanecem constantes durante a aquisição do vídeo. Entretanto, nuvens e objeto cobrindo o Sol podem variar rapidamente, portanto a equalização do histograma deve ser realizada em cada \textit{frame} do vídeo antes de gerar o \textit{timestack}. Estes \textit{frames} equalizados serão utilizadas pelos próximos passos do pré-processamento para gerar o \textit{timestack}. A equalização de histograma é realizada através da biblioteca de processamento gráfico OpenCV, e será discutida com mais detalhes no Capítulo 4.
...
\begin{figure}
  \centering
  \subfloat[\small{\textit{Frame} do vídeo original.}]{\includegraphics[width=0.45\textwidth,keepaspectratio]{simpletimestack_original_untilt.jpg}}
  \qquad
  \subfloat[\small{\textit{Frame} do vídeo com bordas detectadas pelo método de Canny.}]{\includegraphics[width=0.45\textwidth,keepaspectratio]{simpletimestack_canny_untilt.jpg}}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth,keepaspectratio]{simpletimestack_sky_removed_untilt.jpg}
...
\paragraph{}\textit{Thresholding} é o primeiro passo para realizar a segmentação. Neste passo separa-se a região de espuma do restante do mar, alterando todos os \textit{pixels} com intensidade abaixo do limite para 0, e todos os \textit{pixels} acima deste valor para o valor de intensidade máximo. O valor limite é escolhido dinamicamente através do método de Otsu. A figura \ref{FigThreshold} compara a região de espuma detectada após o passo de \textit{threshold} com valores diferentes. Inicialmente foi considerado um método de decisão do valor de \threshold estático, entretanto o método de Otsu produz resultados 
...
\begin{figure}
  \centering
  \subfloat[\small{\textit{Timestack} gerado pelo pré-processamento do vídeo de um dia nublado.}]{\includegraphics[width=.8\textwidth,keepaspectratio]{IMG_0882.jpg}}
  \qquad
  \subfloat[\small{Resultado do \textit{threshold} aplicado ao \textit{timestack} acima.}]{\includegraphics[width=.8\textwidth,keepaspectratio]{process_threshold_fail.jpg}}
  \caption[\small{\textit{Timestack} e \textit{threshold} do vídeo de um dia nublado.}]{\small{\textit{Timestack} e \textit{threshold} do vídeo de um dia nublado.}}
  \label{FigThresholdFail}
\end{figure}
...
\paragraph{}Em condições normais a região de maior área será a região de espuma da imagem, então pode-se descartar todas as outras regiões. Por fim basta identificar as bordas da região restante utilizando um método de detecção de bordas. Novamente, a utilização da biblioteca OpenCV permitiu realizar a detecção de bordas e identificação das regiões isoladas em um único passo, ou seja, não foi necessário realizar mais nenhuma operação para obter as bordas da região de espuma. Outro método considerado para esta tarefa foi o detector de bordas de Canny (figura \ref{FigCannySuzuki}), já utilizado no passo de detecção da linha do horizonte, entretanto pela agilidade fornecida pelo OpenCV o método de Canny foi preterido.

\begin{figure}
  \centering
  \subfloat[\small{Região de espuma detectada pelo método de Canny.}]{\includegraphics[width=0.45\textwidth,keepaspectratio]{process_waveband_canny.png}}
  \qquad
  \subfloat[\small{Região de espuma detectada pelo método de Suzuki (função findContours).}]{\includegraphics[width=0.45\textwidth,keepaspectratio]{process_waveband_suzuki.png}}
  \caption[\small{Comparação dos métodos de Canny e Suzuki para detecção de bordas.}]{\small{Comparação dos métodos de Canny e Suzuki para detecção de bordas.}}
  \label{FigCannySuzuki}
\end{figure}
...
\label{sec:wave_identification}

\paragraph{}O último passo para estimar a altura das ondas em um \timestack é 
identificar as ondas na linha rastreada no passo anterior. Para isso, calcula-se a derivada da sequência discreta $s[n]$, da seguinte forma:
...
\paragraph{}Analisando os dados experimentais foi detectado um problema na identificação do ponto mínimo de algumas ondas. Este problema acontece quando uma onda arrebenta antes do esperado. Ao analisar um vídeo de uma praia, pode-se perceber que a maioria das ondas arrebenta a uma distância constante da areia. Entretanto, algumas ondas, geralmente maiores, podem arrebentar mais distantes da praia. Quando isto ocorre, o nível da região de espuma antes da arrebentação da onda não pode mais ser utilizado para determinar o ponto mínimo da onda. A figura \ref{FigWaveMinPoint}a ilustra o ponto mínimo detectado pelo autômato de uma onda distante da praia. A figura \ref{FigWaveMinPoint}b demostra os limites da onda detectada sobre o \textit{frame} correspondente no vídeo original. Pode-se notar que o ponto mínimo detectado está abaixo do ponto mínimo real para esta onda.

\begin{figure}
  \centering
  \subfloat[\small{Onda detectada em um \textit{timestack} com ponto mínimo detectado erroneamente.}]{\includegraphics[width=.45\textwidth]{wave_min_fix_fail_result.png}}
  \qquad
  \subfloat[\small{Limites da onda detectada sem correção do ponto mínimo.}]{\includegraphics[width=.45\textwidth]{wave_min_fix_fail_wave.jpg}}
  \qquad
  \subfloat[\small{Onda detectada em um \textit{timestack} com ponto mínimo corrigido.}]{\includegraphics[width=.45\textwidth]{wave_min_fix_ok_result.jpg}}
  \qquad
  \subfloat[\small{Limites da onda detectada com ponto mínimo corrigido.}]{\includegraphics[width=.45\textwidth]{wave_min_fix_ok_wave.jpg}}
  \caption[\small{Comparativo entre a detecção dos limites de uma onda distante da areia com e sem a correção do ponto mínimo.}]{\small{Comparativo entre a detecção dos limites de uma onda distante da areia com e sem a correção do ponto mínimo.}}
  \label{FigWaveMinPoint}
\end{figure}

\paragraph{}Para corrigir este problema foi acrescentado um novo fator para determinar o ponto mínimo de uma onda. Após detectar uma onda, mede-se o nível da região de espuma formada pela arrebentação desta onda. O ponto mínimo da onda detectada será a média ponderada entre o ponto mínimo imediatamente antes da onda e o nível da região de espuma formada por ela, com peso 1 para o ponto mínimo e peso 2 para o nível da região de espuma após a onda. Para calcular o nível da região de espuma o próprio autômato pode ser utilizado. Como dito anteriormente, o resultado do autômato é um par de pontos candidato a representar uma onda. Caso este candidato seja descartado como onda, o seu ponto mínimo será utilizado para determinar o nível da região de espuma. A figura \ref{FigWaveMinPoint}c ilustra o ponto mínimo da onda corrigido e os candidatos a onda seguintes descartados pelo autômato. Os pontos vermelhos indicam o ponto mínimo de cada candidato descartado, e os pontos vermelhos indicam o ponto máximo dos candidatos descartados. O nível da região de espuma é calculado através da média aritmética dos três primeiros candidatos após a onda detectada, representado pela linha verde horizontal. A figura \ref{FigWaveMinPoint}d apresenta o resultado da correção aplicada sobre o \textit{frame} equivalente do vídeo original. Pode-se notar que o ponto mínimo detectado está mais próximo do ponto mínimo real, resultando em uma estimação de altura mais precisa.
