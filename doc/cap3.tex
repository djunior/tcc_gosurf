...
\paragraph{}Os métodos apresentados no Capítulo 2 agora serão utilizadas em conjunto para estimar a altura de ondas do mar. Este algoritmo é composto de três etapas: o pré-processamento, o processamento principal, e a análise da imagem resultante. Estas etapas serão detalhadas nas seções a seguir. Antes de detalhá-las, um ponto fundamental para a sua compreensão é entender o que é um \textit{timestack}, qual a sua função e como gerá-lo utilizando um vídeo como base. O \timestack será explicado a seguir.
...
\paragraph{}A equalização de histograma deve ser realizada a cada frame pois a iluminação do sol pode variar durante a aquisição do vídeo.
...
\paragraph{}A detecção do céu é realizada utilizando um detector de bordas de Canny para identificar a borda entre o céu e o mar em um \frame do vídeo (Figura \ref{FigTimestackSkyCanny}). Então, procura-se a primeira borda detectada pelo método de Canny, através do algoritmo descrito em \ref{AlgLineTracking}. A borda detectada é a linha do horizonte. Todos os \textit{pixels} acima dessa linha são considerados como \textit{pixels} da região do céu, e serão removidos no passo seguinte.
...
\paragraph{}O último passo da etapa de pré-processamento é a remoção do céu. Este passo utiliza duas entradas, a linha do horizonte detectada pelo passo anterior, e o \frame equalizado. A partir da linha do horizonte, considera-se todos os \textit{pixels} do \frame equalizado acima dessa linha como preto. 
...
\paragraph{}A primeira etapa de suavização é realizada utilizando um filtro passa-baixas gaussiano, implementado no domínio do espaço com máscara de tamanho 15. Este valor foi escolhido empiricamente,  . Este efeito de suavização é importante para reduzir o nível de ruído na imagem, facilitando a identificação das bordas relevantes nos passos subsequentes. Outro efeito desejado é homogeneizar o nível de intensidade das regiões da imagem, de forma que o passo de \textit{thresholding} consiga segmentar a região de arrebentação do \timestack consistentemente. As figuras \ref{FigGaussianBlurOk} \ref{FigGaussiaBlurFail} ilustram a diferença na segmentação da região de arrebentação com e sem o passo de suavização. A figura \ref{FigGaussianBlurFail} 

%TODO: Adicionar imagem da primeira suavização

\section{\textit{Thresholding}}
\paragraph{}\textit{Thresholding} é o primeiro passo para realizar a segmentação. Neste passo separa-se a região de espuma do restante do mar, alterando todos os pixels com intensidade acima de 150 para 0 

%O valor não é dinâmico por conta daquela história de remoção do céu (figura \ref{FigThreshold}). 
