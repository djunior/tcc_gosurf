...
\paragraph{}A detecção do céu é realizada utilizando um detector de bordas de Canny para identificar a borda entre o céu e o mar em um \textit{frame} do vídeo (Figura \ref{FigTimestackSkyCanny}). Então, procura-se a primeira borda detectada pelo método de Canny, através do algoritmo descrito em \ref{AlgLineTracking}. A borda detectada corresponde a linha do horizonte. Todos os \textit{pixels} acima dessa linha são considerados como \textit{pixels} da região do céu, e serão removidos no passo seguinte.
...
\paragraph{}O último passo da etapa de pré-processamento é a remoção do céu. Este passo utiliza duas entradas, a linha do horizonte detectada pelo passo anterior, e o \textit{frame} equalizado. A partir da linha do horizonte, considera-se todos os \textit{pixels} do \textit{frame} equalizado acima dessa linha como preto. 
...
\begin{figure}[h]
\subfloat[Região de espuma detectada em um \textit{timestack} suavizado.]{
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{waveband_blur.png}
}\par
\subfloat[Região de espuma detectada em um \textit{timestack} não-suavizado.]{
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{waveband_no_blur.png}
}
\caption[\small{Comparação da região de espuma detectada com e sem suaviazação.}]{
  \label{FigGaussianBlurComparison} \small{Comparação da região de espuma detectada com e sem suaviazação.}
}
\end{figure}
...
\paragraph{}\textit{Thresholding} é o primeiro passo para realizar a segmentação. Neste passo separa-se a região de espuma do restante do mar, alterando todos os pixels com intensidade abaixo de 150 para 0, e todos os pixels acima deste valor para o valor de intensidade máximo. O valor limite foi escolhido empiricamente após a análise de diversas imagens, contemplando dias variados. Inicialmente foi considerado um método de decisão do valor de \threshold dinâmico, mas como discutido previamente, os passos de equalização e remoção do céu, realizados durante o pré-processamento permitem utilizar um valor de \threshold estático, indepentende da condição de iluminação no momento da aquisição do vídeo. Entretanto, dias altamente nublados podem causar problemas para o \textit{thresholding}, como pode ser visto na figura \ref{FigThreshold}. Nesta figura os valores de intensidade da região de espuma e da região do mar no entorno das ondas se sobrepõe. Neste caso, o \threshold com valor estático não é capaz de separar as duas regiões, impossibilitando a identificação das ondas do mar nos passos subsequentes.
...
\begin{figure}[h]
\subfloat[\textit{Thresholding} ]{
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{mainprocessing_threshold.jpg}
}\par
\subfloat[fig 2]{
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{mainprocessing_threshold.jpg}
}
\caption[\small{\textit{Timestack} após aplicação de um \textit{threshold}.}]{
  \label{FigThreshold} \small{\textit{Timestack} após aplicação de um \textit{threshold}.}
}
\end{figure}
