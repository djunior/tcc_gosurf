...
\noindent{}A fim de melhorar a organização do projeto, criou uma classe denominada Derivable para implementar as funções 2 e 3 descritas anteriormente. A classe Trajectory herda essas características da classe Derivable. Para representar cada ponto das classes Derivable e Trajectory criou-se uma classe Point. O diagrama \ref{DiagTrajectory} apresenta interface de cada classe e demonstra a relação entre as três.
...
\paragraph{}O cálculo da derivada de uma lista de pontos é implementada no método Derivable::calculateDerivative conforme o código abaixo:
...
%Thresholding
\paragraph{}A seguir é realizada o passo de \textit{thresholding}. Este passo é realizado utilizando a função \textit{threshold}, que recebe cinco parâmetros: um objeto Mat de entrada, um objeto Mat de saída, um valor double indicando o valor limite do \textit{threshold}, um valor double indicando qual é o valor máximo utilizado em \textit{threshold} de binarização, e um valor inteiro indicando qual o tipo de \textit{threshold} que será aplicado. O tipo de \textit{threshold} pode ser combinado com a sinalização cv::CV\_THRESH\_OTSU para utilizar o método de Otsu para calcular o valor do limite. Neste caso, o valor double indicando o limiar de \textit{threshold} deve ser 0. Neste projeto será utilizado o tipo cv::CV\_THRESH\_BINARY com o método de Otsu. O código abaixo exibe como esta função é utilizada:
...
\paragraph{}A função analyseTrajectory implementa o método de correção do ponto mínimo das ondas discutido no capítulo 3. Ela utiliza uma função auxiliar fixBottomPoint que é implementada conforme o código a seguir:
