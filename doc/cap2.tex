...
\paragraph{}A principal vantagem de utilizar um vídeo é eliminar o problema de identificar qual é o melhor momento para analisar a onda fotografada. Utilizando uma sequência de imagens, é mais fácil de determinar qual foi o momento em que a onda estava maior, e ai sim realizar a medição de sua altura. Será demonstrado adiante que o \textit{timestack} também ajuda a tornar as imagens analisadas mais uniformes, criando regiões bem definidas para segmentação. 
...
\paragraph{}Os métodos de aprimoramento de imagens podem ser estudados em dois domínios: domínio espacial e domínio da frequência. O aprimoramento no domínio espacial trabalha com os próprios \textit{pixels} de uma imagem, e podem ser representados pela expressão: \(g(x,y) = T[f(x,y)]\), onde \( g(x,y) \) é a imagem transformada, \( f(x,y) \) é a imagem original e \( T[ \ \cdot\  ] \) é a transformação que será aplicada na imagem original. As técnicas de aprimoramento no domínimo da frequência procuram implementar um filtro que possui uma resposta em frequência específica para realizar a operação desejada, que podem ser representadas pela expressão: $G(j\Omega) = H(j\Omega)F(j\Omega)$, onde $F(j\Omega)$ é a imagem original representada no domínio da frequência, $H(j\Omega)$ é a resposta em frequência do filtro que será aplicado e $G(j\Omega)$ é a imagem resultante desta operação. Em algumas aplicações, trabalhar no domínio da frequência será vantajoso, e em outras será vantajoso trabalhar no domínio espacial. É importante lembra que os métodos no domínio espacial e no domínio da frequência são equivalentes. Neste capítulo todos os métodos serão apresentados no domínio espacial.

\paragraph{}Alguns métodos de aprimoramento de imagens serão utilizados nesse projeto: equalização de histograma, transformações em níveis de cinza, filtros de nitidez (\textit{sharpening}), filtros de suavização (\textit{blur}), filtros de detecção de bordas. Esses métodos são importantes para melhorar o contraste entre as regiões da imagem analisada e reduzir o ruído, facilitando assim o processo de segmentação que será realizado posteriormente.


\paragraph{}A equalização do histograma da imagem é um método utilizado para normalizar a luminosidade da imagem, de forma que seja possível processar tanto dias de alta luminosidade (céu sem nuvens e sol forte) quanto dias de baixa luminosidade (céu nublado e sol fraco). Gonzalez \cite{Gonzalez92} define um histograma de uma imagem digital com 0 até L-1 níveis de cinza como uma função discreta $h(r_{k}) = n_{k}$, onde $r_{k}$ é o k-ésimo nível de cinza e $n_{k}$ é o número de \textit{pixels} com o nível de cinza $r_{k}$. Esta função pode ser normalizada pelo número total de \textit{pixels} $n$, a função normalizada $p(r_{k})$ é definida como:

\[
	p(r_{k}) = \frac{n_{k}}{n}
\]

\noindent{}A função $p(r_{k})$ pode ser interpretada como a probabilidade da ocorrência do nível de cinza $r_{k}$ na imagem digital. A equalização do histograma consiste em encontrar uma transformação T que quando aplicada no histograma normalizado $p(r_{k})$ que resulta em um histograma uniforme. Gonzalez demonstra que a seguinte transformação atinge este objetivo:

\[
	s_{k} = T(r_{k}) = \sum_{j=0}^{k} \frac{n_{j}}{n}
\]

\noindent{}Onde $k = 0,1,2,...,L-1$ e $s_{k}$ é a função que representa o histograma equalizado. 
...
\noindent{}Onde C é uma constante escolhida arbitrariamente. A utilidade dessa função é facilitar a definição das regiões da imagem, quais fazem parte do céu, do mar e da arrebentação. A constante C pode ser calculada através do método de Otsu \cite{Otsu1979}, que calcula um valor de limiar ótimo para binarizar a imagem. Este método considera que o histograma da imagem pode ser dividido em duas classes $C_{0}$ e $C_{1}$, $C_{0}$ englobando todos os \textit{pixels} com níveis de cinza $r_{0}$ até $r_{i}$, e $C_{1}$ englobando todos os \textit{pixels} com níveis de cinza $r_{i+1}$ até $r_{L-1}$. O método de Otsu então calcula o valor de $i$ que minimiza a variância das classes $C_{0}$ e $C_{1}$.
...
\paragraph{}No início do projeto foram pesquisados e utilizados métodos de nitidez, ou \textit{sharpening}. A função desses métodos é aumentar o contraste da imagem na fronteira de regiões, isto é, onde ocorre uma descontinuidade de \textit{pixels}. O método utilizado neste projeto é o método Laplaciano. Segundo Gonzalez \cite{Gonzalez92}, o Laplaciano de uma imagem \(f(x,y)\) é definido por:
...
\noindent{}Onde \(g(x,y)\) é a imagem de saída e \(f(x,y)\) é a imagem de entrada. Entretanto, a medida que o projeto progrediu percebeu-se que o reforço na intensidade das bordas realizado pelos método de \textit{sharpening} não compensavam o aumento de ruído introduzido na imagem, portanto foram descartados do projeto.
...
\paragraph{}O segundo passo do processo de segmentação é aplicar um método de extração de regiões. Inicialmente o método de watershed \cite{Gonzalez92} foi considerado para tal tarefa. O algoritmo de watershed baseia-se na idéia de enxergar a imagem em três dimensões como um vales que serão "inundados", sendo \(x\) e \(y\) coordenadas espaciais e a intensidade de cada pixel \(g(x,y)\) a "altura" de cada ponto neste vale. Os pontos de mínimo locais, ou seja, os pontos onde \(g(x,y)\) é mínimo localmente, consideramos este um ponto mais baixo de um vale, e a região ao redor que será segmentada sua zona de influência. A partir dos pontos de mínimo locais inunda-se a imagem, preenchendo-a com valores gradativamente maiores de intensidade de pixel em sua zona de influência, como seria o nível de água subindo em uma inundação. Eventualmente, a inundação da zona de influência de um ponto mínimo local irá "vazar" para a zona de influência de outro ponto mínimo local. Quando isso ocorre, constrói-se uma "barragem" com largura de um pixel entre as duas zonas de influência, para evitar o vazamento. Quando o nível de inundação atinge o ponto máximo da imagem, isto é, o valor máximo de \(g(x,y)\), o algoritmo de watershed é interrompido. As regiões segmentadas serão definidas pelas "barragens" formadas durante a execução do algoritmo. Na forma descrita acima, o algoritmo de watershed pode levar a segmentação de mais regiões que se desejava originalmente. Isso é corrigido através do uso de marcadores, indicadores de quais são os pontos de mínimo local que serão utilizados pelo algoritmo.

\paragraph{}No decorrer do projeto o método de watershed foi preterido em prol do método de Suzuki \cite{Suzuki85}, um método de rastreamento e hierarquização de bordas em imagens binárias. O método de Suzuki realiza um rastreamento de linha na imagem, procurando por pixels com intensidade diferente de zero. Quando um \textit{pixel} que atende a esta condição é encontrado, é atribuindo um valor para ele, e se rastreia todos os outros \textit{pixels} conectados a ele. Uma vez que toda a borda é detectada, o algoritmo continua o rastreamento na região interna, atribuíndo para estes \textit{pixels} internos valores maiores que os externos. Este processo se repete até que não existam mais pixels internos. O algoritmo é interrompido quando o rastreamento de linha atinge uma das extremidades inferior da imagem, sinalizando que toda a imagem foi rastreada. O resultado deste algoritmo é um conjunto de bordas hierarquizadas. O capítulo 3 apresentará como este método pode ser utilizado para identificar uma região desejada, atingindo o mesmo resultado desejado do método de watershed.
